...
impl SclangType {
    ...
    fn parse_atom(input: &mut &str) -> PResult<Self> {
        alt(("num".value(SclangType::Num), "bool".value(SclangType::Bool))).parse_next(input)
    }

    fn parse_complex(input: &mut &str) -> PResult<Self> {
        alt((Self::parse_record, Self::parse_fun)).parse_next(input)
    }

    fn parse_fun(input: &mut &str) -> PResult<Self> {
        seq!(Self::parse_atom, _: "->", Self::parse_atom)
            .map(|(param_type, return_type)| {
                SclangType::Fun(Box::new(param_type), Box::new(return_type))
            })
            .parse_next(input)
    }
    ...
    fn parse_record(input: &mut &str) -> PResult<Self> {
        delimited(
            ("{", multispace0),
            repeat(1.., Self::parse_record_field).fold(Vec::new, |mut acc, (name, ty)| {
                acc.push((name, ty));
                acc
            }),
            ("}", multispace0),
